...
def storm_optimizer(
    ...
    """
    Create STORM's multi-transform optimizer with separate configs for WM and AC.

    STORM uses different learning rates and gradient clipping for world model
    and actor-critic parameters:
    - World model: lr=1e-4, grad_clip=1000
    - Actor-critic: lr=3e-5, grad_clip=100

    Args:
        wm_config: World model optimizer config (lr, eps, beta1, beta2, grad_clip)
        ac_config: Actor-critic optimizer config (lr, eps, beta1, beta2, grad_clip)

    Returns:
        Optax multi_transform optimizer
    """
    def make_adam_with_clip(config):
    ...
    wm_opt = make_adam_with_clip(wm_config)
    ac_opt = make_adam_with_clip(ac_config)

    # Label function: 'wm' for world model params, 'ac' for policy params
    def label_fn(params):
        """Assign labels to parameter subtrees."""
        from cogniland.agents.world_models.storm.state import STORMParams
        # STORMParams has .wm and .policy subtrees
        return STORMParams(wm='wm', policy='ac')
