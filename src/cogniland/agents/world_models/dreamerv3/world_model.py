...
from cogniland.agents.world_models.dreamerv3.encoder import Encoder
from cogniland.agents.world_models.dreamerv3.decoder import Decoder
from cogniland.agents.world_models.dreamerv3.rssm import RSSM
...
class DreamerV3WorldModel(WorldModel):
    ...
    def __init__(
        ...
        # Extract flatten_keys from encoder config for consistency
        ...
        # Ensure both encoder and decoder have consistent flatten_keys
    ...
    def init_params(
        ...
        for key, spec in obs_space.items():
            ...
            # Check if this key is in flatten_keys (will be processed as vector)
            is_flattened = key in self.flatten_keys

            if 'image' in key.lower() and not is_flattened:
