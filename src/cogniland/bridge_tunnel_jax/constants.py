"""Constants for the pure-JAX bridge_tunnel env.

This is a JAX port of ``src/cogniland/bridge_tunnel/`` (PyTorch). The tile
...
See ``tests/test_bridge_tunnel_jax_parity.py`` for the equivalence proof.

Natural-only vocabulary: obsidian + cue tiles are retired; TREE is the sole
inviolable tile. Must stay in lock-step with ``bridge_tunnel/tiles.py``.
"""
from __future__ import annotations

# ── tile ids (mirror src/cogniland/bridge_tunnel/tiles.py) ────────────────
GRASS = 0
WATER = 1
ROCK = 2
WOOD = 3
