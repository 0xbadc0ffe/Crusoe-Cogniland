"""Map dataset loader for the pure-JAX bridge_tunnel env.

The numpy ``cogniland.bridge_tunnel.mapgen.generate_bridge_tunnel_map`` stays the
...
2. Precomputes, per map, the **static-terrain** min-action cost-to-go field
   (the SAME ``BridgeTunnelEnv._compute_ctg`` Dijkstra the PyTorch env uses for its
...
from cogniland.bridge_tunnel.mapgen import MapRecord, generate_bridge_tunnel_map, is_reachable
from cogniland.bridge_tunnel.tiles import TARGET


# Canonical natural task. The goal is a 3-cell central door on the right wall
# (goal_half=1 -> rows [mid-1, mid+1]); this is the default environment. Trees
# are edge-biased in mapgen so wall-hugging to the door is discouraged.
# (goal_half=None would make the whole wall a goal; larger N a wider door.)
NATURAL_KWARGS = dict(
    size=32,
    width=64,
    orientation="natural",
...
def generate_map_dataset(
    ...
    **map_kwargs,
    ...
    held-out validation seeds (10_000+) used by ``make_bridge_tunnel_val_maps.py``.
    """
