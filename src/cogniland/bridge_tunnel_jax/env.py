"""Gymnax-style ``Environment`` wrapper around the pure-JAX bridge_tunnel dynamics.
...
class BridgeTunnelJaxEnv(environment.Environment[EnvState, EnvParams]):
    """The pure-JAX bridge_tunnel env (natural maps)."""
    ...
    def default_params(self) -> EnvParams:
        if self._default_params is None:
            raise RuntimeError(
                "BridgeTunnelJaxEnv needs an EnvParams with map arrays. Use "
                "`bridge_tunnel_jax.maps.load_map_arrays(...)` + "
