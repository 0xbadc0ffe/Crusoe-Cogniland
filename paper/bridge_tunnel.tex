...
\item \textbf{Enforced skill $=$ implicit, irreversible commitment.} The action
space is \code{Discrete(6)} (four moves, \code{BUILD}, \code{MINE}). Commitment is
\emph{implicit}: the \textbf{first successful} \code{BUILD} (bridging a faced water
cell) locks the agent into building for the rest of the episode; the first
successful \code{MINE} locks it into mining. Thereafter the opposite tool is
permanently disabled --- using it is a no-op with a small penalty. So the agent
commits simply by \emph{choosing to cross its first obstacle}; there is no separate
commit action it could spam blind at the spawn, so the commitment naturally tracks
the first obstacle it decides to cross (usually the category's dominant one). A
\code{BUILD}/\code{MINE} that faces the wrong or empty tile while still uncommitted
is a harmless no-op and does not commit. A one-time \emph{commit cost} is charged on
the committing action, so when a short detour exists it can be better never to
commit (leaving a small \textsf{none} mass). The commitment is exposed in the
observation scalars ($\mathbb{R}^{7}=[\textsc{facing}(4),\ \text{step}/\text{max},\
\text{committed\_build},\ \text{committed\_mine}]$).
\end{itemize}

\paragraph{The belief$\to$skill map.} Crossing the \emph{abundant} obstacle of a
category is cheaper than detouring around all of it, so the better commitment is
biased by the category: \textsc{lakes} favour committing to \emph{build},
\textsc{rocky} to \emph{mine}, \textsc{balanced} either. Because commitment is
implicit (the first obstacle the agent crosses), and on a one-sided map that first
crossing is usually the dominant obstacle, the commitment naturally correlates with
the category. We read this off as a $3\times3$ \textbf{commit matrix}
(Fig.~\ref{fig:commit-matrix}): rows are the map category, columns are the skill the
agent ended the episode committed to (\textsf{none}/\textsf{build}/\textsf{mine}),
and each entry is the fraction of held-out episodes of that category --- so every
row sums to $1$. We are after a \emph{soft} bias toward the matching tool (e.g.
lakes leaning to build, rocky to mine) with a little \textsf{none} mass, not a hard
diagonal --- crossing is never strictly mandatory, since a detour usually exists.
