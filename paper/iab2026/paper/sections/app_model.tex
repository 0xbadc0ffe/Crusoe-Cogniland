...
\caption{\textbf{The agent.} The $21\times21$ tile view is one-hot encoded with
    ...
    three layers of convolutions, concatenated with five scalars ('facing' variable one-hot(4) and time passed (1)).
    This vector is projected to a
    $256$-d embedding; The observation embedding is fed to a GRU with $128$-d hidden state $\mathbf{h}_t$. The Actor and Critic are just linear projections of $\mathbf{h}_t$}
    \label{fig:model}
\end{figure}
\begin{table}[t]
\centering
