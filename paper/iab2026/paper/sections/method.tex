...
$\mathbf{v}$ influences the flag decision, we steer the agent along this direction and leave every other
direction untouched,
$\mathbf{h}'=\mathbf{h}+(b_{\lambda}-\mathbf{h}^{\top}\mathbf{v})\,\mathbf{v}$,
setting it to a point between the map's own category and the other,
$b_{\lambda}=(1-\lambda)\,b^{\star}_{\text{own}}+\lambda\,b^{\star}_{\text{other}}$,
where $\lambda$ is the dose: $\lambda=0$ writes the agent's own category, a
null write that should change nothing, and $\lambda=1$ writes the opposite
category. The write is applied at corridor entry and
the outcome is the decision $D$ as a function of $\lambda$. As a control we apply the same displacement along random directions.
