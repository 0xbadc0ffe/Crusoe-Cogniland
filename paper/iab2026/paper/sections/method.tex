...
\emph{Belief:} the agent's estimate of which kind of map it is on,
$P(c\mid o_{1:t})$. A well-calibrated belief is necessary to achieve high reward: a
Bayes-optimal agent walks to the flag of the most probable category,
and on balanced maps either flag is optimal.
The agent carries and updates its belief through the recurrent state
$\mathbf{h}_t$; we read it out as a scalar $\hat b$ (Section~\ref{sec:decoding}).
\emph{Behavior:} the number of times the agent bridged water and tunnelled
rock. Obstacles can be crossed with a tool or walked around (Appendix~\ref{app:measures}).
\emph{Decision:} $D\in\{\text{top},\text{bottom}\}$, the flag the agent
walks into at the end of the episode. On lakes and rocky maps only one flag
gives the terminal reward, so a wrong decision is visible in the return. On
balanced maps both flags give the reward, so the decision does not change the
return: a corrupted belief can move $D$ without any drop in reward.
A steering intervention edits the hidden state $\mathbf{h}_t$ to change the
behavior, for instance to suppress \textsc{mine} so that the agent walks
around mountains instead. The ideal intervention is selective: it changes
only the behavior it targets and leaves every other capability of the agent
as it was. For this task that means the belief, and hence the final decision
$D$, must be the same as in the unsteered episode on the same map: the agent
walks around the mountain and still goes to the flag it would have chosen. We
measure \emph{belief corruption} as the change in $\hat b$ and in $D$ between
the steered and the unsteered episode.
...
direction untouched,
$\mathbf{h}'=\mathbf{h}\pm \alpha\,g\,\mathbf{v}$,
where $g=b^{\star}_{\text{rocky}}-b^{\star}_{\text{lakes}}$ is the distance between
the two category poles, a positive edit pushes toward rocky and a negative edit toward lakes. $\alpha=0$ is the unsteered agent and $\alpha=1$ moves the
...
\label{sec:steering}
Interventions edit $\mathbf{h}_t$ directly, as popular steering methods do \citep{turner2023actadd,rimsky2024caa,zou2023repe}. To suppress the skill
...
below $\theta$ are left untouched, so $\theta$ sets the strength and $\eta$
only the step size; a fixed steering vector is the unconditional, one-step
special case. Behavior change is the
