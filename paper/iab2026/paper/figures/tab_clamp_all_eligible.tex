...
\caption{\textbf{Steering behavior corrupts the belief} Gated gradient based steering on maps where both tools are used at least once. \emph{\# bridges} and \emph{\# tunnels} are successful tool uses per episode, the lowest of each category in bold; \emph{timeout} is the ratio of episodes that didn't reach a flag. \emph{rocky flag} and \emph{lakes flag} show the agent's decision; $\hat b(\text{rocky})$ is the mean probability of the rocky category read from the belief scalar by the logistic probe. Failure modes are highlighted in red.}
\label{tab:clamp_all}
