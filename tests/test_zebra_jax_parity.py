"""Parity test: pure-JAX zebra_nav == PyTorch ZebraNavEnv.
...
VAL_MAPS = Path(__file__).resolve().parents[1] / "data" / "zebra_nav" / "val_maps.pkl"
...
def test_ctg_matches_oracle():
    """The precomputed ctg field equals ZebraNavEnv._compute_ctg exactly."""
    records, _ = _load_records()
    for rec in records[:6]:
        arrays = records_to_arrays([rec])
        oracle = ZebraNavEnv._compute_ctg(rec.terrain, rec.target).astype(np.float32)
