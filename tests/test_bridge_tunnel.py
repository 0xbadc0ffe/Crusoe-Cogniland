"""Tests for the bridge_tunnel env (natural-only vocabulary).
...
from cogniland.bridge_tunnel import BridgeTunnelEnv, generate_bridge_tunnel_map, tiles as T
from cogniland.bridge_tunnel.env import (
    A_DOWN, A_LEFT, A_MINE, A_PLACE, A_RIGHT, A_UP,
    F_DOWN, F_LEFT, F_RIGHT, F_UP,
)
from cogniland.bridge_tunnel.mapgen import is_reachable
...
def test_non_natural_orientation_rejected():
    for bad in ("diagonal", "vertical", "mixed"):
        with pytest.raises(ValueError):
            generate_bridge_tunnel_map(seed=0, orientation=bad)
...
def test_natural_map_contract(seed: int):
    ...
    rec = generate_bridge_tunnel_map(seed=seed, orientation="natural", size=48)
    ...
    rec2 = generate_bridge_tunnel_map(seed=seed, orientation="natural", size=32, width=64, goal_half=4)
    gc = rec2.terrain[:, 63]
...
def test_central_door_default():
    ...
    rec = generate_bridge_tunnel_map(seed=5, orientation="natural", size=32, width=64, goal_half=1)
    gc = rec.terrain[:, 63]
...
def test_natural_rectangular_and_coverage():
    ...
    rec = generate_bridge_tunnel_map(seed=1, orientation="natural", size=32, width=64,
...
def test_natural_edge_bands_clear():
    ...
    rec = generate_bridge_tunnel_map(seed=3, orientation="natural", size=32, width=64)
    H, W = rec.terrain.shape
...
def test_natural_sand_and_dirt_present():
    ...
    rec = generate_bridge_tunnel_map(seed=2, orientation="natural", size=32, width=64)
...
def test_trees_hug_top_and_bottom():
    ...
    top = bot = mid = 0
    for s in range(40):
        rec = generate_bridge_tunnel_map(seed=s, orientation="natural", size=H, width=64,
                                  tree_frac=0.06)
...
def _fresh_env(seed: int = 0) -> BridgeTunnelEnv:
    env = BridgeTunnelEnv(seed=seed)
    env.reset()
    return env
...
def test_env_rejects_non_natural_orientation():
    for bad in ("diagonal", "vertical", "mixed"):
        with pytest.raises(ValueError):
            BridgeTunnelEnv(orientation=bad)
...
def _custom_env(terr, spawn, target, **kw):
    from cogniland.bridge_tunnel.mapgen import MapRecord
    rec = MapRecord(terrain=terr, spawn=spawn, target=target, seed=0)
    H = terr.shape[0]
    env = BridgeTunnelEnv(size=H, width=terr.shape[1], view_size=3,
...
def test_handcrafted_solver_reaches_target():
    ...
    env = BridgeTunnelEnv(seed=0, max_steps=600)
    obs, info = env.reset()
