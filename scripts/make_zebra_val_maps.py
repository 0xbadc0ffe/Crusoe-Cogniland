#!/usr/bin/env python3
"""Curate a fixed set of zebra_nav validation maps and pickle them.

The same pickled set is used by the demo (``play_zebra.py --maps``) and by
...
from cogniland.zebra_nav.mapgen import is_reachable  # noqa: E402
...
def main():
    ...
    p.add_argument("--goal-half", type=int, default=4, help="natural: central goal door half-height (<0 = whole wall)")
    ...
    for j in range(len(recs), len(axes)):
        axes[j].axis("off")
    fig.suptitle(f"zebra_nav validation maps — {args.orientation} {args.env_size}x{args.env_width}", fontsize=11)
    fig.tight_layout()
