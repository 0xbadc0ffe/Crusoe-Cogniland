...
Usage
-----
    python scripts/crafter/play_cogniland.py --size 64
"""
...
# allow `python scripts/crafter/play_cogniland.py` from the repo root without install
...
# Demo-maps dataset produced by ``scripts/crafter/generate_demo_maps.py``.
...
def _draw_pick_map(win, font, small_font, big_font,
    ...
    if not maps:
        msg = font.render("No maps in data/demo_maps/. Run scripts/crafter/generate_demo_maps.py.", True, (240, 120, 120))
        win.blit(msg, (40, 120))
        pygame.display.flip()
        return rects
