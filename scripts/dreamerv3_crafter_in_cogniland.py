"""DreamerV3 on crafter_in_cogniland — single-file, pure-JAX.

Mirrors ``purejaxwm/dreamerv3_cogniland.py`` 1:1; env-specific changes:
...
* Score: ``score/success`` = success-rate (mean(reached) over recent
  episodes) — the shared key both PPO and Dreamer log under so they're
  comparable on a single W&B chart.

The model size presets follow Table 3 of the DreamerV3 paper:

| Preset  | d   | deter (8d) | cnn_d / codes (d/16) |
|---------|-----|------------|----------------------|
| 12M     | 256 | 1024 (×4)  | 16                   |  (small)
...
from cogniland.crafter_in_cogniland import (  # noqa: E402
...
def _make_env_params(cfg) -> EnvParams:
    ...
    if not maps_path.exists():
        ...
        arrays = generate_map_dataset(
            n_maps=cfg["num_train_maps"], size=cfg["map_size"],
            seed=cfg.get("map_gen_seed", 0),
        )
...
def _save_final_checkpoint(final_state, run_dir: Path, env_step: int) -> None:
    """Save final params per OUTPUT_PROTOCOL.md v0."""
...
def _default_cfg() -> dict:
    """Paper-aligned defaults; ``--size`` resolves the model-size block."""
    return {
        # env
...
def main():
    ...
    # CLI overrides
    for k in ("total_env_steps", "num_envs", "train_ratio", "map_size",
              "view_size", "wandb_project", "wandb_mode", "maps_path"):
        v = getattr(args, k.replace("_", "_"), None)
        # argparse stores dashed names as underscored attrs
    ...
    def log_cb(metrics_dict, env_step):
        ...
        for k, v in metrics_dict.items():
            ...
            try:
                ...
                continue
        # 100-episode rolling success / return for nice plots
        if _success_history:
