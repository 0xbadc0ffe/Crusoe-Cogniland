#!/usr/bin/env python3
"""Steered evaluation of a bridge_tunnel_commit PPO agent: push the GRU hidden
state toward the SUB-optimal skill (mine on lakes maps, build on rocky maps) and
measure whether the auxiliary *belief* readout moves along with the behavior.
...
* ``suppress-skill``   each env step, take up to ``--clamp-iters`` unit-norm
  steps of size α down the closed-form actor-head gradient of
...
linear too, so per env step it ascends the closed-form belief-head gradient
of log P(wrong archetype) (rocky on lakes maps, lakes on rocky maps) in
unit-norm steps of size α until P(wrong) ≥ ``--belief-floor``. Unlike the
fixed class-mean/belief-head axis push, the edit is minimal and
state-dependent (samples already believing the target are untouched), and
the actor is never referenced — behavior movement is the measured side
effect.
...
def batched_rollout_steered(policy, rec, n_traj, view_size, max_steps, device,
    ...
    the GRU hidden each step and (b) recording the belief softmax per step.
...
def main():
    ...
    p.add_argument("--alphas", default="0.25,0.5,1.0,2.0",
                   help="comma-separated steering strengths; each entry is one "
                        ...
                        "headline figures. With the unit-norm class-mean "
                        "direction, α≈0.25 is already 'slight' (belief mostly "
                        "flips, behavior barely moves) and α≥1 saturates the "
                        "belief and starts breaking the policy. For "
