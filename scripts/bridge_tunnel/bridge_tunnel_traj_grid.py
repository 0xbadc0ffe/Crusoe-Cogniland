#!/usr/bin/env python3
"""200-trajectory stochastic-policy grids for a trained bridge_tunnel agent.
...
``n_traj`` rollouts × stripes.
    ...
    python scripts/bridge_tunnel/bridge_tunnel_traj_grid.py \\
...
def batched_rollout(policy, rec, n_traj, view_size, max_steps, device):
    ...
    envs = [BridgeTunnelEnv(map_record=rec, size=H, width=W, view_size=view_size,
...
def main():
    ...
    dummy = BridgeTunnelEnv(size=env_size, width=env_width, view_size=view_size)
    dummy.reset()
    _sd = ckpt["policy"]
    obs_enc = cargs.get("obs_encoding", "embed")
    if "tile_embed.weight" in _sd:                                 # embed: rows = NUM_TILES
        n_tiles = int(_sd["tile_embed.weight"].shape[0])
    else:                                                          # onehot: K = conv in-ch − 2 (CoordConv)
    ...
    policy = PPOGRUPolicy(dummy.observation_space, num_actions=n_act,
                          ...
                          num_tile_classes=n_tiles,
                          obs_encoding=obs_enc).to(device)
    ...
    for j in range(args.n_maps):
        ...
        rec = generate_bridge_tunnel_map(size=env_size, width=env_width, seed=seed,
