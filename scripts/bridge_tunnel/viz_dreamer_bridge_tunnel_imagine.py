...
``dreamerv3_bridge_tunnel.py`` built them (config from ``<run>/config.json``),
...
Outputs per example (j = 0..n-1, eval map seed = eval_seed_start + j):
  ...
  <out-dir>/imagine_strip_seed<seed>.png   (filmstrip of every Nth frame)
    ...
    python scripts/bridge_tunnel/viz_dreamer_bridge_tunnel_imagine.py \\
...
# ── Crafter-sprite rendering (mirrors scripts/bridge_tunnel/play_bridge_tunnel.py) ───────────
...
# Exact copies of dreamerv3_bridge_tunnel.{BridgeTunnelEncoder,BridgeTunnelDecoder} so restored
