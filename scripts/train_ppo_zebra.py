#!/usr/bin/env python3
"""PPO + GRU for the zebra_nav POMDP, with W&B logging.

The env (``cogniland.zebra_nav.ZebraNavEnv``) is a 32×32 (or 64×64) Crafter-
style navigation task: BL→TR across diagonal *zebra stripes* of water and
...
-------------------------
    ...
    python scripts/train_ppo_zebra.py \\
...
from cogniland.zebra_nav import ZebraNavEnv  # noqa: E402
from cogniland.zebra_nav.tiles import NUM_TILES  # noqa: E402


# =============================================================== vec env

class VecZebraEnv:
    """Synchronous vector env around N ZebraNavEnv instances, auto-resetting
    and reporting per-episode stats on the done step."""

    def __init__(self, num_envs: int, **env_kwargs):
        base_seed = env_kwargs.pop("seed", 0)
        self.envs = [ZebraNavEnv(seed=base_seed + i, **env_kwargs) for i in range(num_envs)]
...
def main():
    ...
    parser.add_argument("--total-timesteps", type=int, default=2_000_000,
                        help="zebra maps converge well before 2M")
    # reward
    ...
    parser.add_argument("--wandb-project", default="zebra_nav")
    ...
    run_name = args.run_name or f"ppo_zebra_size{args.env_size}_seed{args.seed}_{int(time.time())}"
    wandb.init(
        project=args.wandb_project, name=run_name, config=vars(args),
        mode=args.wandb_mode, save_code=True,
        tags=["algo=ppo_gru", f"map={args.env_size}", "env=zebra_nav",
