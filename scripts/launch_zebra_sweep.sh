#!/usr/bin/env bash
# Launch N parallel W&B sweep agents for the zebra_nav PPO sweep on the local
...
# Usage: scripts/launch_zebra_sweep.sh <entity/project/sweep_id> [N=9] [COUNT=1]
...
set -uo pipefail
SWEEP="${1:?usage: launch_zebra_sweep.sh <entity/project/sweep_id> [N] [COUNT]}"
N="${2:-9}"
COUNT="${3:-1}"
...
for i in $(seq 1 "$N"); do
  ...
  NUMEXPR_NUM_THREADS=1 PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=0 \
    nohup "$WANDB" agent --count "$COUNT" "$SWEEP" > "sweep_logs/zebra_agent_${i}.log" 2>&1 &
  echo "agent $i -> pid $!"
  sleep 4
done
echo "launched $N agents (count=$COUNT each → up to $((N*COUNT)) runs) for $SWEEP"
echo "logs: sweep_logs/zebra_agent_*.log"
