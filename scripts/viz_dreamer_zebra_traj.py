#!/usr/bin/env python3
"""200-trajectory stochastic-policy grids for a trained DreamerV3 zebra_nav agent.

The sibling of ``scripts/zebra_traj_grid.py`` (the PPO renderer): for each of
...
It reconstructs the world model + actor EXACTLY as ``dreamerv3_zebra_nav.py``
...
It uses the SAME 6 maps the PPO grid uses — ``generate_zebra_map(size=32,
...
generator, proven bit-for-bit in tests/test_zebra_jax_parity.py).

    python scripts/viz_dreamer_zebra_traj.py \\
        --checkpoint runs/dreamerv3_zebra_nav_natural_25M_1780145070/checkpoints/step_1000000 \\
...
from cogniland.zebra_nav_jax.render import build_obs  # noqa: E402
from cogniland.zebra_nav_jax import dynamics as dyn  # noqa: E402
...
# Exact copy of dreamerv3_zebra_nav.ZebraEncoder so the restored params (named
...
def batched_rollout(encoder, rssm, actor_head, wm_params, ac_params,
                    env: ZebraNavJaxEnv, params: EnvParams,
...
def main():
    ...
    p.add_argument("--checkpoint", type=Path,
                   default=Path("runs/dreamerv3_zebra_nav_natural_25M_1780145070/"
    ...
    env = ZebraNavJaxEnv()   # params passed explicitly per panel (single map)
