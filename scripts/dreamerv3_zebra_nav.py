"""DreamerV3 on the zebra_nav NATURAL-maps task — single-file, pure-JAX.

Trains the in-tree ``purejaxwm`` DreamerV3 on EXACTLY the task the PyTorch
PPO+GRU ``natural_agent`` was trained on (``models/zebra_nav/natural_agent.yaml``):
...
``cogniland.zebra_nav_jax`` port, proven bit-for-bit equivalent to the PyTorch
``ZebraNavEnv`` in ``tests/test_zebra_jax_parity.py``.
...
def _resolve_maps_path(cfg) -> Path:
    ...
    return Path(
        f"data/zebra_nav_jax/train_natural_n{cfg['num_train_maps']}.pkl"
    )
