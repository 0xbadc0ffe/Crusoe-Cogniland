...
This implementation uses a modular architecture:
- WorldModel protocol (DreamerV3WorldModel)
...
from cl.agents.agent import Agent
...
from cl.agents.commons.visualization import video_predict, visualize_buffer_task_distribution
from cl.agents.utils import RatioTracker
from cl.agents.world_models.dreamerv3.world_model import DreamerV3WorldModel
from cl.agents.world_models.dreamerv3.state import DreamerV3Params, DreamerV3TrainState
...
from cl.agents.policy.actor_critic import imagine_trajectory, imag_loss, repl_loss
from cl.agents.utils import sg


@register_agent('dreamerv3')
def make_dreamer(config: OmegaConf, obs_space: Dict, act_space: int) -> Agent:
    """Factory function that creates a DreamerV3 agent.

    DreamerV3 agent using modular architecture.

    Architecture:
        - World Model: DreamerV3WorldModel (encoder, decoder, RSSM, reward, cont)
    ...
    Returns:
        Agent instance with DreamerV3 policy
    """
    cfg = config.agent
    ...
    # Debug mode flag for diagnostic metrics
    debug_mode = cfg.get('debug', False)

    # Environment names for buffer task distribution visualization
    env_names = list(OmegaConf.to_container(config.envs, resolve=True))
    ...
    # Create world model with configs from OmegaConf
    ...
    world_model = DreamerV3WorldModel(
        ...
        rssm_config=OmegaConf.to_container(cfg.model.dyn.rssm, resolve=True),
    ...
    # Create optimizer
    optimizer = laprop(**OmegaConf.to_container(cfg.model.opt, resolve=True))

    # Initialize training ratio tracker
    batch_size = cfg.batch_size
    batch_length = cfg.batch_length
    ...
    # Imagination and replay loss configs
    ...
    viz_interval = cfg.visualization.interval
    viz_context = cfg.visualization.context
    viz_log_prefix = cfg.visualization.log_prefix
    ...
    def _loss_fn(
        ...
        """Combined loss function for DreamerV3."""
        wm_params = params.wm
        ...
        # Process sequence through world model with scan
        initial_state = world_model.initial_state(wm_params, batch_size_local)

        def step_fn(carry, inputs):
            prev_state, rng_local = carry
            embed, action, is_first_t = inputs

            if rng_local is not None:
                rng_local, step_rng = jax.random.split(rng_local)
            else:
                step_rng = None

            post_state, prior_state = world_model.observe(
                wm_params, prev_state, action, embed, is_first_t,
                training=True, rng=step_rng
            )

            kl_dict = world_model.rssm.apply(
                wm_params.dynamics,
                post_state,
                prior_state,
                method=world_model.rssm.kl_divergence
            )
            kl_dyn = kl_dict['dyn']
            kl_rep = kl_dict['rep']

            return (post_state, rng_local), (post_state, kl_dyn, kl_rep)

        if grad_checkpoint:
            step_fn_checkpointed = jax.checkpoint(step_fn)
            _, (states, dyn_losses, rep_losses) = jax.lax.scan(
                step_fn_checkpointed,
                (initial_state, rng_wm),
                (embeds.swapaxes(0, 1), actions.swapaxes(0, 1), is_first.swapaxes(0, 1))
            )
        else:
            _, (states, dyn_losses, rep_losses) = jax.lax.scan(
                step_fn,
        ...
        states = jax.tree.map(lambda x: x.swapaxes(0, 1), states)
        dyn_loss = dyn_losses.swapaxes(0, 1).mean()
        rep_loss = rep_losses.swapaxes(0, 1).mean()

        feat = world_model.get_feat(wm_params, states)
        feat_flat = feat.reshape(batch_size_local * seq_len, -1)

        states_flat = jax.tree.map(
            lambda x: x.reshape(batch_size_local * seq_len, *x.shape[2:]),
            states
        )
        ...
        cont_loss = cont_loss_flat.reshape(batch_size_local, seq_len)

        losses['rec'] = rec_loss
        losses['rew'] = rew_loss
        losses['con'] = cont_loss
        losses['dyn'] = jnp.ones((batch_size_local, seq_len)) * dyn_loss
        losses['rep'] = jnp.ones((batch_size_local, seq_len)) * rep_loss

        # Diagnostic metrics (only logged when debug mode is enabled)
        if debug_mode:
            # Reconstruction accuracy (for categorical observations)
            if 'image' in recons and hasattr(recons['image'], 'logits'):
                pred_logits = recons['image'].logits
                pred_classes = jnp.argmax(pred_logits, axis=-1)
                target_classes = jnp.argmax(obs_flat['image'], axis=-1)
                metrics['debug/rec_pixel_accuracy'] = float((pred_classes == target_classes).mean())

                # Class distribution in predictions (10 classes after remapping)
                num_classes = pred_logits.shape[-1]
                for c in range(num_classes):
                    metrics[f'debug/pred_class_{c}_freq'] = float((pred_classes == c).mean())
        ...
        start_states = jax.tree.map(lambda x: x[:, -K:], states)

        def reshape_for_imag(x):
            shape = x.shape
            if len(shape) == 3:
                return x.reshape(batch_size_local * K, shape[-1])
            elif len(shape) == 4:
                return x.reshape(batch_size_local * K, shape[-2], shape[-1])
            return x.reshape(-1, *shape[2:])

        start_states_flat = jax.tree.map(reshape_for_imag, start_states)

        def policy_fn(features, rng_action):
            rng_action, rng_sample = jax.random.split(rng_action)
            action_dist = policy.apply_actor(
                policy_params.actor, features, training=True, rng=None
            )
    ...
    def _train_step_core(
        train_state: DreamerV3TrainState,
        batch: Dict[str, jnp.ndarray],
        rng: jax.random.PRNGKey,
    ) -> Tuple[DreamerV3TrainState, Dict]:
    ...
    def _select_action_jit(
        ...
        wm_state = jax.tree.map(reset_where_first, init_state, wm_state)
    ...
    def init(rng: jax.random.PRNGKey) -> AgentState:
        ...
        # Get feature dimension from RSSM config
        rssm_config = cfg.model.dyn.rssm
        deter_dim = rssm_config.deter
        stoch_size = rssm_config.stoch
        num_classes_cfg = rssm_config.classes
        feat_dim = deter_dim + stoch_size * num_classes_cfg
    ...
    def train(
        ...
        while frames_collected < num_train_frames:
            ...
            if state.runtime.current_task_id is not None:
                replay_transition['task_id'] = np.full(
                    env.num_envs, int(state.runtime.current_task_id), dtype=np.int32
                )
               ...
               buf_stats.get('valid_timesteps', 0) >= batch_length:
                ...
                if viz_enabled:
                    if state.runtime.train_steps % viz_interval == 0:
                        ...
                        buf_stats_viz = buffer.stats(state.runtime.buffer_state)
                        visualize_buffer_task_distribution(
                            buffer_state=state.runtime.buffer_state,
                            valid_size=buf_stats_viz['size'],
                            env_names=env_names,
                            step_count=int(state.runtime.train_steps),
                        )
    ...
    def state_from_checkpoint(checkpoint_data: Dict, runtime_state: RuntimeState) -> AgentState:
        ...
        params = DreamerV3Params(wm=wm_params, policy=policy_params_ckpt)

        train_state_obj = DreamerV3TrainState(
    ...
    def reset(state: AgentState, rng: jax.random.PRNGKey) -> AgentState:
        ...
        new_train_state = DreamerV3TrainState.create(
            apply_fn=None,
            params=state.train_state.params,
            tx=optimizer,
    ...
    def set_environment(
        ...
        """Update state for a new environment."""
        ...
        if task_id is not None:
            new_runtime = new_runtime.replace(
                current_task_id=jnp.array(task_id, dtype=jnp.int32)
            )
        if new_runtime is not state.runtime:
            return AgentState(train_state=state.train_state, runtime=new_runtime)
        return state
